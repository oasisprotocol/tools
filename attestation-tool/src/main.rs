...
lazy_static::lazy_static! {
    ...
    static ref IAS_REPORT_SIGNING_CERTIFICATE: Vec<u8> =
        pem::pem_to_der(include_str!("../data/test_report_signing_cert"),
...
async fn main() {
    ...
    if let (Some(client_certificate), Some(pass)) = (
        matches.value_of("CERTIFICATE"),
        matches.value_of("CERTIFICATE_PASS"),
    ) {
    ...
    let spid = match matches.value_of("SPID") {
        ...
        None => ias_client
            .get_spid(&report.as_ref()[..REPORT_SIZE_TRUNCATED])
            .await
            .unwrap(),
    };

    let sig_rl = ias_client
        .get_sig_rl(&quote.gid(), Some(&spid))
        .await
        .unwrap_or(vec![]);
    ...
    match ias_client.verify_quote(quote.quote(), Some("early")).await {
        Ok(response) => {
            ...
            println!(
                "    signature type: {}",
                report.isv_enclave_quote_body().signature_type
            );
            println!(
                "    gid: {}",
                HexPrint(&report.isv_enclave_quote_body().gid)
            );
            println!(
                "    isvsvn qe: {}",
                report.isv_enclave_quote_body().isvsvn_qe
            );
            println!(
                "    isvsvn pce: {}",
                report.isv_enclave_quote_body().isvsvn_pce
            );
            println!(
                "    basename: {}",
                HexPrint(&report.isv_enclave_quote_body().basename)
            );
            ...
            println!(
                "  epid_pseudonym: {}",
                report.epid_pseudonym().as_ref().map_or_else(
            ...
            if pstatus.is_some()
                || response.advisory_url.is_some()
                || response.advisory_ids.is_empty()
            {
                println!("");
            }
