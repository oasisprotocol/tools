...
func openDatabase(path string) (*DB, error) {
	...
	opts.MemTableSize = 1 << 20    // 1MB memtable (minimal) - v3 uses MemTableSize instead of MaxTableSize
	...
	// BadgerDB v3 always uses mmap for memtables, which doesn't work on FUSE filesystems
