...
type EVMDataInfo struct {
	...
	BytecodeSize int    `json:"bytecode_size,omitempty"`  // Size of contract bytecode
	BytecodeRaw  string `json:"bytecode_raw,omitempty"`   // First 32 bytes of raw bytecode (hex)
}
...
type EVMEventInfo struct {
	Address        string   `json:"address"`                    // H160 contract address (hex, 40 chars)
	TopicCount     int      `json:"topic_count"`                // Number of topics (0-4)
	Topics         []string `json:"topics,omitempty"`           // H256 topics array (hex, 64 chars each)
	EventSignature string   `json:"event_signature,omitempty"`  // Human-readable signature (if known)
	EventHash      string   `json:"event_hash,omitempty"`       // topic[0] keccak256 hash
	DataSize       int      `json:"data_size"`                  // Size of non-indexed data
	DataRaw        string   `json:"data_raw,omitempty"`         // First 64 bytes of raw data (hex)
...
type EVMTxInputInfo struct {
	TxHash      string       `json:"tx_hash"`                // Transaction hash (hex, 64 chars)
	BatchOrder  uint32       `json:"batch_order"`            // Order within batch
	InputSize   int          `json:"input_size"`             // Raw input size
...
type EVMCallInfo struct {
	...
	Address string `json:"address,omitempty"`   // H160 target address (hex, 40 chars)
	...
	DataRaw string `json:"data_raw,omitempty"`  // First 32 bytes of raw data (hex)
}
...
type EVMTxOutputInfo struct {
	OutputSize int    `json:"output_size"`           // Raw output size
	Success    bool   `json:"success"`               // Transaction succeeded
	ResultSize int    `json:"result_size,omitempty"` // Size of result data
	ResultRaw  string `json:"result_raw,omitempty"`  // First 32 bytes of raw result (hex)
