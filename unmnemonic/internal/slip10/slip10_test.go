...
func (output *hkdOutput) Check(t *testing.T, s *Secret, c *ChainCode) {
	if x := hex.EncodeToString(c[:]); x != output.chainCode {
		t.Fatalf("chain code mismatch (Got %v)", x)
	}

	privateKey := ed25519.NewKeyFromSeed(s[:])
	if x := hex.EncodeToString(privateKey[:32]); x != output.privateKey {
...
func (vec *testVector) Run(t *testing.T) {
	...
	t.Run("Master", func(t *testing.T) {
		...
		if k, c, err = NewMasterKey(seed); err != nil {
	...
	for _, v := range vec.children {
		...
		t.Run(fmt.Sprintf("Child_%d", index), func(t *testing.T) {
			if k, c, err = NewChildKey(k, c, index); err != nil {
...
func TestVectors(t *testing.T) {
	for i, v := range testVectors {
		t.Run(fmt.Sprintf("TestVector_%d", i), func(t *testing.T) {
			v.Run(t)
		})
		// v.Run(t)
	}
