...
func decodeRuntimeMkvsValue(keyType string, value []byte) *RuntimeMkvsValueInfo {
	...
	if keyType != "node" {
		// Decode write_log values
		if keyType == "write_log" {
			var writeLog WriteLog
			if err := cbor.Unmarshal(value, &writeLog); err != nil {
				info.RawError = err.Error()
			} else {
				info.WriteLog = writeLog
			}
			info.NodeType = "write_log"
			return info
		}
	...
	case 0x00: // LeafNode: [2-byte keySize LE][key][4-byte valueSize LE][value]
		...
		module := decodeRuntimeModuleKey(key)
...
func headerTypeName(headerType uint8) string {
	...
	case 4:
		return "Suspended"
	default:
		return fmt.Sprintf("Unknown_%d", headerType)
	}
}
