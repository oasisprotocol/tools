...
type EVMDataInfo struct {
	...
	StorageSlot   string `json:"storage_slot,omitempty"`    // H256 (32 bytes) - storage slot (0x-prefixed hex)
	...
	BlockHash     string `json:"block_hash,omitempty"`      // H256 (32 bytes) - block hash (0x-prefixed hex)
	BytecodeSize  int    `json:"bytecode_size,omitempty"`   // Size of contract bytecode
	BytecodeRaw   string `json:"bytecode_raw,omitempty"`    // First 32 bytes of raw bytecode (0x-prefixed hex)
}
...
type EVMEventInfo struct {
	...
	DataRaw        string   `json:"data_raw,omitempty"`         // First 64 bytes of raw data (0x-prefixed hex)
...
type EVMCallInfo struct {
	...
	DataRaw  string `json:"data_raw,omitempty"`  // First 32 bytes of raw data (0x-prefixed hex)
}
...
type EVMTxOutputInfo struct {
	...
	ResultRaw   string `json:"result_raw,omitempty"`  // First 32 bytes of raw result (0x-prefixed hex)
